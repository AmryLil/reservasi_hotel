...
@section('content')
    <div class="rounded-xl w-full bg-white p-6 shadow-md">
        <h1 class="text-2xl font-bold mb-6 text-blue-800">Edit User</h1>
        ...
        <form action="{{ route('admin.users.update', $user->email_222320) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="nama_222320" class="block font-semibold mb-1">Nama</label>
                    <input type="text" name="nama_222320" id="nama_222320"
                        value="{{ old('nama_222320', $user->nama_222320) }}"
                        class="w-full border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        required>
                </div>
                <div>
                    <label for="email_222320" class="block font-semibold mb-1">Email</label>
                    <input type="email" name="email_222320" id="email_222320"
                        value="{{ old('email_222320', $user->email_222320) }}"
                        class="w-full border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        required>
                </div>
                <div>
                    <label for="phone_222320" class="block font-semibold mb-1">Phone</label>
                    <input type="text" name="phone_222320" id="phone_222320"
                        value="{{ old('phone_222320', $user->phone_222320) }}"
                        class="w-full border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        required>
                </div>
                <div>
                    <label for="alamat_222320" class="block font-semibold mb-1">Alamat</label>
                    <input type="text" name="alamat_222320" id="alamat_222320"
                        value="{{ old('alamat_222320', $user->alamat_222320) }}"
                        class="w-full border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        required>
                </div>
                <div>
                    ...
                    <select name="gender_222320" id="gender_222320"
                        ...
                        <option value="male" {{ old('gender_222320', $user->gender_222320) == 'male' ? 'selected' : '' }}>
                            Male</option>
                        <option value="female"
                            {{ old('gender_222320', $user->gender_222320) == 'female' ? 'selected' : '' }}>Female</option>
                    </select>
                </div>
                <div>
                ...
                <div>
                    <label for="password_222320" class="block font-semibold mb-1">Password (Kosongkan jika tidak ingin
                        mengubah)</label>
